...
- **文案质检**: 浏览器直连 Gemini 公开 API 或 OpenRouter（用户自备 API Key），不经过后端
- **图片生成/匹配**: FastAPI (Python) 代理 Google Vertex AI API，Workload Identity 认证（无需 API 密钥）
- **模型链（图片匹配，Vertex）**: gemini-2.5-flash-lite → gemini-2.5-flash（故障转移）
...
**实现位置**: `src/utils/copyAudit.ts` - `parseCopy()` 函数（由 `src/App.tsx` 的 `handleAuditCopy()` 调用）
...
### 文案质检 API（客户端直连，不经过后端）

**实现位置**: `src/utils/copyAudit.ts` - `runCopyAudit()`，由 `src/App.tsx` 的 `callGeminiDirect()` / `callOpenRouterText()` 注入实际的模型调用。

用户在侧边栏「AI 引擎配置」中选择 Gemini 或 OpenRouter 并填入自己的 API Key：

- **Gemini**: 浏览器直接 `POST https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={geminiApiKey}`，模型链 `gemini-2.5-flash → gemini-2.5-flash-lite`（故障转移）。
- **OpenRouter**: 浏览器直接 `POST https://openrouter.ai/api/v1/chat/completions`，使用侧边栏选择的模型 ID。

`runCopyAudit()` 将文案分段（每批 15 段）、拼装质检 prompt、逐批调用注入的 `callModel(prompt)`，并解析返回的 JSON 数组为 `AuditResult[]`。

### AI 模型配置

**文案质检（Gemini 分支）**：`gemini-2.5-flash` → `gemini-2.5-flash-lite`（故障转移，浏览器直连）
**文案质检（OpenRouter 分支）**：用户在侧边栏选择的模型
**图片匹配/生成（Vertex，经后端 `/api/generate`）**：`gemini-2.5-flash` → `gemini-2.5-flash-lite`（故障转移）
